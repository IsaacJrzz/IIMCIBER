    ...
    public void analyze(List<SimProcess> processes) {
        ...
        for (SimProcess p : processes) {
            ...
            if (blacklist.contains(p.getName())) {
                logger.log("PROCESOS", "Proceso \"" + p.getName() + "\" está en lista negra (pid=" + p.getPid() + ").");
            }

            //  CPU excesiva
            if (p.getCpu() >= cpuThreshold) {
                logger.log("PROCESOS", "Proceso \"" + p.getName() + "\" excede uso de CPU (" +
            ...
            if (suspicious.contains(p.getName())) {
                ...
                if (aliveSecs >= persistenceSeconds) {
                    logger.log("PROCESOS", "Proceso sospechoso \"" + p.getName() +
                            "\" persiste " + aliveSecs + "s (pid=" + p.getPid() + ").");
                }
            }
        }

        // limpiamos PIDs que ya no existen
